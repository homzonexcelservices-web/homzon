...
export default function AdminLeaveManagement() {
  ...
  const fetchLeaves = async () => {
    setLoading(true);
    try {
      const res = await fetch("http://localhost:4000/api/leave/admin");
  ...
  const handleAction = async (id, status) => {
    try {
      const res = await fetch(`http://localhost:4000/api/leave/admin/update/${id}`, {
        method: "PUT",
    ...
    <div style={{ maxWidth: "1000px", margin: "20px auto", padding: "20px" }}>
          ...
          {leaves.length > 0 ? (
            leaves.map((l) => (
              <tr key={l._id}>
                ...
                <td style={{ border: "1px solid #ddd", padding: 8 }}>{l.hrComments || "—"}</td>
            ...
            <tr>
              <td colSpan={7} style={{ textAlign: "center", padding: 20 }}>
                No leaves pending Admin approval
              </td>
            </tr>
          )}
        </tbody>
      ...
      {selectedLeave && (
        <div style={{ marginTop: 20, padding: 20, border: "1px solid #ccc", borderRadius: 8 }}>
          ...
          <p><strong>HR Comments:</strong> {selectedLeave.hrComments || "None"}</p>
          <textarea
