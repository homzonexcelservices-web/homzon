...
export default function AdminAdvanceManagement() {
  ...
  const fetchAdvances = async () => {
    setLoading(true);
    try {
      const res = await fetch("http://localhost:4000/api/advance/admin");
  ...
  const handleAction = async (id, status) => {
    try {
      const res = await fetch(`http://localhost:4000/api/advance/admin/update/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          status,
          comments,
          approverName: localStorage.getItem("name") || "Admin",
        }),
      });
      const data = await res.json();
      alert(data.message);
    ...
    <div style={{ maxWidth: "1000px", margin: "20px auto", padding: "20px" }}>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", marginTop: 20 }}>
        <thead>
          <tr style={{ background: "#e0e7ff" }}>
            ...
            <th style={{ border: "1px solid #ccc", padding: 8 }}>Modified Amount</th>
            <th style={{ border: "1px solid #ccc", padding: 8 }}>Reason</th>
            <th style={{ border: "1px solid #ccc", padding: 8 }}>HR Comments</th>
          ...
          {advances.length > 0 ? (
            advances.map((a) => (
              <tr key={a._id}>
                ...
                <td style={{ border: "1px solid #ddd", padding: 8 }}>{a.modifiedAmount ? `₹${a.modifiedAmount}` : "—"}</td>
                <td style={{ border: "1px solid #ddd", padding: 8 }}>{a.reason}</td>
                <td style={{ border: "1px solid #ddd", padding: 8 }}>{a.hrComments || "—"}</td>
            ...
            <tr>
              <td colSpan={7} style={{ textAlign: "center", padding: 20 }}>
                No advances pending Admin approval
              </td>
            </tr>
          )}
        </tbody>
      ...
      {selectedAdvance && (
        <div style={{ marginTop: 20, padding: 20, border: "1px solid #ccc", borderRadius: 8 }}>
          ...
          <p><strong>Original Amount:</strong> ₹{selectedAdvance.amount}</p>
          <p><strong>Modified Amount:</strong> {selectedAdvance.modifiedAmount ? `₹${selectedAdvance.modifiedAmount}` : "None"}</p>
          <p><strong>Reason:</strong> {selectedAdvance.reason}</p>
          <p><strong>HR Comments:</strong> {selectedAdvance.hrComments || "None"}</p>
          <textarea
